...
A model repository is a filesystem based repository of models and configuration schema used by the Triton Inference Server. While a server can use one or more model repositories, for the purposes of simplicity, this explanation only uses a single repository. The general structure of a model repository is as follows:
```
<model-repository>/
  <model-name>/
    [config.pbtxt]
...
* `config.pbtxt`: For each model, users need to define the model configuration. This configuration, at minimum, needs to define the name of the platform, and the name, shape, and datatype of the input and output layers.
* `version`: versioning allows for multiple versions of the same model to be available for use depending on the policy selected.
...
With the models and the filestructure ready, the only point of dicussion remaining are the `config.pbtxt` files. As an example, consider the model configuration for the `EAST text detection` model. This is an ONNX model that has one `input` and two `output` layers.
...
* **input and output layers**: The input and output layers specify the name, shape datatype and more, while providing operations like reshaping and more.
